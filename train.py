...
#        print(idx)
        ...
        loss_focal = focal_loss(logit, one_hot(labels.unsqueeze(dim=1).to(device).long(), num_classes=6))
        ...
        total_loss = 0.5 *loss + diceloss + 0.5 *loss_focal
        # total_loss = loss + loss_focal
        # total_loss = loss
...
if __name__ == "__main__":
    ...
    parser.add_argument('--network_weights_path', default = "/home/user02/TUTMING/ming/Aero/result/unetcps_model_lr142.pth", help = 'Path to save Network weights')

    ### Use GPU or not
    parser.add_argument('--use_cuda', default=True, help = 'use GPUs?')

    ### Hyperparameters
    parser.add_argument('--batch-size', default = 48, type = int, help = 'Number of images sampled per minibatch?')        # ours 48
    ...
    parser.add_argument('--epochs', default = 150, type = int, help = 'Maximum number of epochs?')
    ...
    valset = AeroCLoader(set_loc = 'right', set_type = 'test', size = 'small', \
