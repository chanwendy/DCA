...
class LoveDA(Dataset):
    ...
    * https://arxiv.org/abs/2110.08733
    ...
    def __init__(
        ...
        Args:
            root: root directory where dataset can be found   Path = 'data'
            split: one of "train", "val", or "test"           str = 'train'
            scene: specify whether to load only 'urban', only 'rural' or both            list[str] = ['urban', 'rural']
            transforms: a function/transform that takes input sample and its target as
                entry and returns a transformed version                   Callable[[dict[str, Tensor]], dict[str, Tensor]] | None = None
    ...
    def __getitem__(self, index: int):
        ...
        if self.transforms is not None:
            ...
            image  = torch.FloatTensor(data["image"].transpose(2, 0, 1))
    ...
    def _load_image(self, path) -> Tensor:
        """Load a single image.
    ...
    def _load_target(self, path) -> Tensor:
    ...
    def plot(self, sample: dict[str, Tensor], suptitle) -> Figure:
